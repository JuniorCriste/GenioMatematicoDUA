...
// Estado do Jogo
let filaCharadas = [];
let score = 0, lives = 5, currentAnswer = "", timer = null, timeLeft = 0, totalTime = 0;

window.onload = () => {
    loadHighScore();
    iniciarSetupAcessibilidade();
};

// --- FUNÇÕES DO TUX (ACESSIBILIDADE) ---
...
function iniciarSetupAcessibilidade() {
    const setupArea = document.getElementById('setup-content');
    ...
    const etapaFonte = () => {
        ...
        const finalizarEtapa = (escolha) => {
            if(escolha === 2) { configAcess.fonteGrande = true; document.body.classList.add('font-grande'); }
            etapaContraste();
        };
        document.getElementById('f1').onclick = () => finalizarEtapa(1);
        document.getElementById('f2').onclick = () => finalizarEtapa(2);
        window.onkeydown = (e) => { if(e.key === '1') finalizarEtapa(1); if(e.key === '2') finalizarEtapa(2); };
    };

    const etapaContraste = () => {
        setupArea.innerHTML = `
        ...
        const finalizarEtapa = (escolha) => {
            if(escolha === 2) { configAcess.altoContraste = true; document.body.classList.add('high-contrast'); }
            etapaNarracao();
        };
        document.getElementById('c1').onclick = () => finalizarEtapa(1);
        document.getElementById('c2').onclick = () => finalizarEtapa(2);
        window.onkeydown = (e) => { if(e.key === '1') finalizarEtapa(1); if(e.key === '2') finalizarEtapa(2); };
    };

    const etapaNarracao = () => {
        setupArea.innerHTML = `
            <p class="game-title-mini">Deseja utilizar o narrador com audiodescrição?</p>
        ...
        const finalizarEtapa = (escolha) => {
            configAcess.narracao = (escolha === 1);
            setupArea.innerHTML = `<button id="btn-start" class="postit-button">JOGAR (Pressione 5)</button>`;
            document.getElementById('btn-start').onclick = iniciarProcesso;
            window.onkeydown = (e) => { if(e.key === '5') iniciarProcesso(); };
        };
        document.getElementById('n1').onclick = () => finalizarEtapa(1);
        document.getElementById('n2').onclick = () => finalizarEtapa(2);
        window.onkeydown = (e) => { if(e.key === '1') finalizarEtapa(1); if(e.key === '2') finalizarEtapa(2); };
    };
...
function iniciarProcesso() {
    ...
    const interval = setInterval(() => {
        loadPercent += 5;
        document.getElementById('load-progress').style.width = loadPercent + "%";
        if (loadPercent >= 100) { clearInterval(interval); startGame(); }
    }, 50);
}
...
function nextQuestion() {
    ...
    if(configAcess.narracao) tuxDiz("Pergunta: " + qObj.q);
...
function handleError() {
    ...
    if(configAcess.narracao) tuxDiz(`Incorreto. Você agora tem ${lives} vidas.`);
...
function endGame() {
    ...
    const interval = setInterval(() => {
        ret--;
        document.getElementById('return-timer').innerText = ret;
        if (ret <= 0) { clearInterval(interval); location.reload(); }
    }, 1000);
}
...
async function takeSnap() {
    ...
    } catch (e) { console.warn("Câmera OFF"); }
}
